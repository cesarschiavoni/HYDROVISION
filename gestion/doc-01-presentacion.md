...
· cwsi_formula.py: implementación de Jackson et al. (1981) con coeficientes Bellvert et al. (2016) calibrados para Malbec. Tres índices implementados: CWSI (Jackson 1981), Índice Jones/Ig (Jones 1999, validado en vid por Gutiérrez et al. 2018, PLoS ONE), y predicción de potencial hídrico de tallo ψ_stem [MPa] (Pires et al. 2025, R²=0.663 en capturas de tarde). Resultados coherentes con rangos publicados para condiciones de Cuyo: CWSI 0.0–0.95, ψ_stem −0.35 a −1.49 MPa. Error CWSI por NETD ~100mK = ±0.008 con 28 píxeles foliares promediados (dentro del límite ±0.07 de Araújo-Paredes et al. 2022).
· thermal_pipeline.py: pipeline completo frame MLX90640 (32×24 px) → segmentación foliar P20–P75 → CWSI ponderado por fracción foliar. Metodología multi-angular (7 posiciones de gimbal × 3 ventanas horarias: 6 fijas + 1 condicional viento). Flag canopy_side norte/sur (Pires 2025, Zhou et al. 2022).
...
Infraestructura de riego — inversión previa al TRL 4: el viñedo experimental opera actualmente con riego por canal (acequia). La implementación del plan experimental TRL 4 —que requiere 5 filas de calibración con regímenes hídricos diferenciados de 100% ETc a déficit hídrico severo— exige la instalación de riego por goteo con solenoides Rain Bird. Esta conversión (canal → goteo por fila con control individual por fila) constituye una inversión de infraestructura que el equipo fundador asume como parte de la contrapartida del proyecto, independientemente del financiamiento ANPCyT. La instalación será ejecutada por Javier y Franco Schiavoni previo al inicio de la campaña experimental (Mes 3–4 del proyecto financiado).
...
Validación en condiciones de campo real controlado: 10 filas de Malbec de 136m (1.360 vides) organizadas en dos zonas — Zona de Calibración (filas 1–5, 5 regímenes hídricos: F1=0% ETc, F2=15%, F3=40%, F4=65%, F5=100% ETc) y Zona de Producción (filas 6–10, todas 100% ETc, nodos en modo comercial autónomo con pipeline completo CWSI→HSI→alerta→recomendación de riego), sistema de riego por goteo diferencial con 10 solenoides Rain Bird (1 por fila — todas las filas 1–10). 10 nodos permanentes (5 calibración + 5 producción, 1 por fila, planta central), gimbal motorizado 7 ángulos × 96 ciclos/día. Potencial hídrico de tallo verificado con bomba de Scholander bajo protocolo Dra. Monteoliva. Error CWSI predicho vs. medido < ±0.10 unidades.
