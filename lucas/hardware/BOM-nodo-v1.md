...
> **COGS de referencia (lote 50 unidades):** USD 149/nodo (incluye shelter SHT31, tubo colimador IR y termopar foliar).
> **Arquitectura TRL4:** ESP32-S3 DevKit + módulos breakout I2C/SPI + MicroPython — sin PCB custom.
> Con volumen 500+ unidades: COGS baja a ~USD 121 (arquitectura revierte a bare chip + PCB custom de producción).
...
| 5 | **Anemómetro** | RS485 Modbus RTU, 0–60 m/s, IP65 | 1 | 20–35 | AliExpress "wind speed sensor RS485 modbus rtu IP65" (elegir ≥ 200 ventas, ≥ 4.5★) | Mide velocidad del viento para corregir el CWSI. El viento produce enfriamiento convectivo artificial de la hoja. El firmware aplica rampa gradual 4-12 m/s (14-43 km/h): el peso del CWSI se reduce linealmente de 35% a 0%. A ≥12 m/s (43 km/h): `wind_override=true` → HSI = 100% MDS. Las 9 capas de mitigación física (sotavento, shelter, tubo colimador, termopar) extienden el rango útil del CWSI de 4 a 8 m/s. Sin este sensor, el sistema daría falsas alarmas en días de Zonda en San Juan (~15-30 días/temporada). | Config.h: `WIND_RAMP_LO 4.0f`, `WIND_RAMP_HI 8.0f`, `RS485_REG_WIND 0x0000`. **Verificar con el datasheet del sensor comprado** que el registro 0x0000 es velocidad — algunos modelos usan 0x0001. MAX485 (fila 13) es el transceiver. Driver: `driver_anemometro.h`. Comprar 2 (spare). |
...
- `wind_override`: `true` cuando viento ≥ 12 m/s (43 km/h) → w_cwsi=0, w_mds=1. Entre 4-12 m/s rampa gradual
