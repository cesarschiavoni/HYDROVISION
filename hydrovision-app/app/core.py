"""
core.py — Estado compartido, helpers y servicios
HydroVision AG

Contiene:
  - _GDD_MODELS, _KY_FAO56: datos de fenologia y coeficientes FAO-56
  - ZONES, _NODE_IRRIGATION, _NODE_SOL_SIM: estado en memoria del campo
  - Funciones de GDD, fenologia y calculo de Ky
  - Helpers de DB: zonas, nodos, seed, reload
  - _SatelliteFusionService: fusion nodo-Sentinel-2
  - Utilidades: _vpd_kpa, _stress_label
"""
...
def _reload_zones(db: Session) -> None:
    """Reconstruye el dict ZONES desde zone_config (muta in-place para que
    todos los módulos que importaron ZONES vean los cambios)."""
    ...
    ZONES.clear()
    ZONES.update(new_zones)
    print(f"[core._reload_zones] Cargadas {len(ZONES)} zonas en memoria")
...
# ---------------------------------------------------------------------------
# Procesamiento de telemetría (reutilizado por /ingest HTTP y consumer MQTT)
# ---------------------------------------------------------------------------
...
def process_ingest(payload_dict: dict, db, ip: str = None) -> dict:
    """
    Procesa un payload de telemetría de nodo.
    Recibe un dict con la misma estructura que NodePayload.
    Retorna dict con la respuesta para el nodo (downlink).

    Usado por:
      - POST /ingest (HTTP fallback)
      - MQTT consumer (hydrovision/+/telemetry)
    """
    from app.models import AuditLog, IrrigationLog, NodeConfig, Telemetry
    from app.deps import check_rate_limit, verify_hmac, audit

    node_id = payload_dict["node_id"]

    # Rate limiting
    if not check_rate_limit(node_id):
        audit(db, "rate_limit", node_id=node_id, ip=ip)
        return {"status": "rejected", "reason": "rate_limit"}

    # Calidad de captura
    calidad = payload_dict.get("calidad_captura", "ok")
    if calidad != "ok":
        return {"status": "descartado", "motivo": calidad}

    # Extraer campos
    gps = payload_dict.get("gps", {})
    lat = gps.get("lat")
    lon = gps.get("lon")
    env = payload_dict.get("env", {})
    thermal = payload_dict.get("thermal", {})
    dendro = payload_dict.get("dendro", {})
    hsi = payload_dict.get("hsi", {})
    sol_data = payload_dict.get("solenoid")

    sol_canal = sol_data.get("canal", 0) if sol_data else None
    sol_canal_db = sol_canal if sol_canal and sol_canal > 0 else None

    _ensure_node_config(db, node_id, lat=lat, lon=lon, solenoid=sol_canal_db)

    db.add(Telemetry(
        node_id=node_id, ts=payload_dict.get("ts", 0),
        cwsi=thermal.get("cwsi", 0), hsi=hsi.get("value", 0),
        mds_mm=dendro.get("mds_mm", 0), t_air=env.get("t_air", 0),
        rh=env.get("rh", 0), wind_ms=env.get("wind_ms", 0),
        rain_mm=env.get("rain_mm", 0), bat_pct=payload_dict.get("bat_pct", 0),
        calidad=calidad, origen="real",
        lat=lat, lon=lon,
    ))

    # Riego
    inhibir_riego = False
    if sol_data and sol_data.get("canal", 0) > 0:
        sol_active = sol_data.get("active", False)
        nc = db.query(NodeConfig).filter(NodeConfig.node_id == node_id).first()

        if nc and nc.zona_id and _zona_en_reposo(nc.zona_id):
            if sol_active:
                inhibir_riego = True
                _NODE_IRRIGATION[node_id] = False
                audit(db, "irrigate_inhibit", node_id=node_id,
                      detail=f"zona={nc.zona_id} reposo_fenologico", ip=ip)
            else:
                _NODE_IRRIGATION[node_id] = False
        else:
            prev_active = _NODE_IRRIGATION.get(node_id, False)
            _NODE_IRRIGATION[node_id] = sol_active

            if sol_active != prev_active:
                db.add(IrrigationLog(
                    node_id=node_id,
                    zona=nc.zona_id if nc else None,
                    duration_min=sol_data.get("ciclos_activo", 0) * 15,
                    active=sol_active,
                ))
                evt = "irrigate_on" if sol_active else "irrigate_off"
                audit(db, evt, node_id=node_id,
                      detail=f"zona={nc.zona_id if nc else '?'} canal={sol_data['canal']}", ip=ip)

    db.add(AuditLog(event="ingest_ok", node_id=node_id, ip=ip))
    db.commit()

    # Respuesta (downlink al nodo)
    resp = {"status": "ok", "node_id": node_id}

    nc_final = db.query(NodeConfig).filter(NodeConfig.node_id == node_id).first()
    ...
    if _NODE_SOL_SIM.get(node_id, False):
        resp["sol_sim"] = True
