...
> **Para qué sirve:** Evalúa el checkpoint entrenado contra el dataset Scholander y genera métricas (MAE, RMSE, R²) + gráficos (scatter CWSI, Bland-Altman, scatter ψ_stem, error por sesión). Target TRL 4: MAE < 0.10, R² > 0.60 vs CWSI real, R² > 0.75 vs ψ_stem.
...
- Genera `investigador/models/edge/hydrovision_cwsi_int8.tflite` (cuantizado INT8 para RPi)
